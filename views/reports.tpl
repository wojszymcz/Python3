...
<script>
    ...
    var wybranaPostac = document.getElementById("postacie").value;
     ...
     // Sprawdź, czy istnieje już link, jeśli tak, usuń go
    var existingLink = document.getElementById("pobierzLink");
    if (existingLink) {
    ...
    // Dodaj nowo utworzony element przed istniejącym hr
    var hrElement = document.querySelector('.container hr');
    hrElement.parentNode.insertBefore(a, hrElement);
...
<style>
  /* Center the anchor with the ID 'raportLink' */
  #pobierzLink {
    display: block;
